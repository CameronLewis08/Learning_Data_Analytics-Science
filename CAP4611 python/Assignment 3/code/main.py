...
def q2_1():
    ...
    n = X.shape[0]
    v = np.ones(n)
    v[400:] = 0.1  # Last 100 points are outliers
    ...
    test_and_plot(
        model, X, y, title="Weighted Least Squares",
        filename="weighted_least_squares.pdf"
    )
...
def q2_4_1():
    ...
    test_and_plot(
        ...
        filename="robust_regression.pdf",
    )
...
def q2_4_2():
    ...
    plt.figure()
    plt.plot(fs_gd, label='GradientDescent', marker='o', markersize=3)
    plt.plot(fs_gd_ls, label='GradientDescentLineSearch', marker='s', markersize=3)
    ...
    filename = Path("..", "figs", "learning_curves.pdf")
    print("Saving to", filename)
    plt.savefig(filename)

    print(f"GradientDescent final objective: {fs_gd[-1]:.4f}")
    print(f"GradientDescentLineSearch final objective: {fs_gd_ls[-1]:.4f}")

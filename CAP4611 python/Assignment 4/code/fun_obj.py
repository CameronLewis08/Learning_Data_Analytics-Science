...
class LeastSquaresLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        m_residuals = (
            y_hat - y
        )  # minus residuals, slightly more convenient here
...
class SoftmaxLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        # Reshape w from vector (of length k*d) to k x d matrix
...
class SoftmaxLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        # Compute the linear predictions: X @ W.T has shape (n, k)
        # where element (i, c) is w_c^T @ x_i
        XW = X @ W.T  # shape: (n, k)

        # For numerical stability, subtract max from each row
        XW_stable = XW - np.max(XW, axis=1, keepdims=True)

        # Compute exp(w_c^T @ x_i) for all i, c
        exp_scores = np.exp(XW_stable)  # shape: (n, k)

        # Compute sum over all classes for each example
        sum_exp = np.sum(exp_scores, axis=1, keepdims=True)  # shape: (n, 1)

        # Compute log(sum(exp(...)))
        log_sum_exp = np.log(sum_exp)  # shape: (n, 1)

        # Extract the score for the true class of each example
        true_class_scores = XW_stable[np.arange(n), y]

        # Compute loss: sum over all examples of [-w_{y_i}^T @ x_i + log(sum(exp(...)))]
        f = np.sum(-true_class_scores + log_sum_exp.flatten())

        # Compute gradient
        # p(y_i = c | W, x_i) = exp(w_c^T @ x_i) / sum(exp(w_{c'}^T @ x_i))
        probs = exp_scores / sum_exp  # shape: (n, k)

        # Create indicator matrix: 1(y_i = c) for all i, c
        indicators = np.zeros((n, k))
        indicators[np.arange(n), y] = 1

        # Gradient: sum over i of x_i * [p(y_i = c | W, x_i) - 1(y_i = c)]
        # This gives us a k x d gradient matrix
        gradient_diff = probs - indicators  # shape: (n, k)
        G = gradient_diff.T @ X  # shape: (k, d)

        # Flatten gradient back to vector form to match input w format
        g = G.reshape(-1)

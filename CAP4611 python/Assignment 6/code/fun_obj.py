...
def log_sum_exp(Z):
    ...
    return Z_max + np.log(
        np.sum(np.exp(Z - Z_max[:, None]), axis=1)
    )  # per-colmumn max
...
class FunObj:
    ...
    def check_correctness(self, w, *args, epsilon=1e-6):
        ...
        # Check the gradient
        if (
            np.max(np.abs(difference)) / np.linalg.norm(estimated_gradient)
            > 1e-6
        ):
            raise Exception(
...
class SoftmaxLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        Xw_of_y = XW[
            np.arange(n), y
        ]  # n; confusingly, : would NOT work in first index
...
class CollaborativeFilteringZLoss(FunObj):
    ...
    def evaluate(self, z, W, Y):
        ...
        # Set NaN residuals to 0

        R_masked = np.copy(R)
        R_masked[np.isnan(R_masked)] = 0

        # Compute loss
        f = 0.5 * np.sum(R_masked ** 2) + (self.lammyZ / 2) * np.sum(Z ** 2)

        # Compute gradient
        g = R_masked @ W.T + self.lammyZ * Z
...
class CollaborativeFilteringWLoss(FunObj):
    ...
    def evaluate(self, w, Z, Y):
        ...
        # Compute loss
        f = 0.5 * np.sum(R_masked ** 2) + (self.lammyW / 2) * np.sum(W ** 2)

        # Compute gradient
        g = Z.T @ R_masked + self.lammyW * W
...
class RobustPCAFeaturesLoss(FunObj):
    ...
    def evaluate(self, z, W, X):
        ...
        # Compute gradient
        grad_multiplier = R / np.sqrt(R ** 2 + self.epsilon)
        g = grad_multiplier @ W.T
...
class RobustPCAFactorsLoss(FunObj):
    ...
    def evaluate(self, w, Z, X):
        ...
        # Compute residuals R = ZW - X
        R = Z @ W - X

        # Compute smooth L1 loss: f = Σ √(R² + ε)
        f = np.sum(np.sqrt(R ** 2 + self.epsilon))

        # Compute gradient: ∇W = Z^T [R ⊘ √(R² + ε)]
        # Element-wise division of R by √(R² + ε)
        grad_multiplier = R / np.sqrt(R ** 2 + self.epsilon)
        g = Z.T @ grad_multiplier

...
class LeastSquaresBias:
    ...
    def fit(self, X, y):
        n = X.shape[0]
        # Add column of ones to the left of X for bias term
        X_bias = np.hstack([np.ones((n, 1)), X])

        # Solve normal equations with augmented X
        self.w = solve(X_bias.T @ X_bias, X_bias.T @ y)

    def predict(self, X_pred):
        n = X_pred.shape[0]
        # Add column of ones to match training
...
class LeastSquaresPoly:
    ...
    def fit(self, X, y):
         ...
         # Fit least squares on transformed featureas
    ...
    def predict(self, X_pred):
        ...
        # Predict using least squares model
        return self.leastSquares.predict(Z_pred)
    ...
    def _poly_basis(self, X):
        n = X.shape[0]
        # Initialize Z with appropriate size
        Z = np.zeros((n, self.p + 1))

        # Fill each column with x^j for j = 0, 1, ..., p

    ...
    def fit(self, X, y):
        ...
        while not np.all(selected):
            ...
            for j in range(d):
                ...
                score, _ = self.global_loss_fn.evaluate(w_on_sub, X[:, selected_with_j], y)
            ...
            if min_loss < old_loss:  # something in the loop helped our model
                ...
                print(
                    f"adding feature {best_feature:>3} - loss {min_loss:>7.3f}"
                )
            else:
...
class LeastSquaresClassifier:
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_classes):
            ...
            self.W[i] = np.linalg.solve(
                X.T @ X + 0.0001 * np.eye(d), X.T @ ytmp
            )
...
class LinearClassifierOneVsAll(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        k = len(y_classes)
        assert set(y_classes) == set(
            range(k)
        )  # check labels are {0, 1, ..., k-1}

        # quick check that loss_fn is implemented correctly
        self.loss_fn.check_correctness(
            np.zeros(d), X, (y == 1).astype(np.float32)
        )
        ...
        # For each class, fit a binary classifier
        for c in range(k):
            # Create binary labels: +1 for class c, -1 for all others
            y_binary = np.ones(n) * -1
            y_binary[y == c] = 1

            # Fit the binary classifier for class c
            w_init = np.zeros(d)
            W[c], *_ = self.optimize(w_init, X, y_binary)
...
class MulticlassLinearClassifier(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        # Initial guess: flatten k x d matrix into vector of length k*d
        w_init = np.zeros(k * d)

        # Optimize - this will work with the flattened w
        self.w, self.fs, self.gs, self.ws = self.optimize(w_init, X, y)

        # Reshape w back to k x d matrix for storage and later use
        self.W = self.w.reshape(k, d)


    def predict(self, X_hat):
        """YOUR CODE HERE FOR Q3.4"""
        scores = X_hat @ self.W.T

        # Return the class with highest score for each example

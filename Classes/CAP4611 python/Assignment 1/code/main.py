...
def q5_1():
    ...
    fifth = np.percentile(arr, 5)
    twentyFive = np.percentile(arr, 25)
    fifty = np.percentile(arr, 50)
    seventyFive = np.percentile(arr, 75)
    ninetyFive = np.percentile(arr, 95)
    ...
    print(
        ...
        "\n** 75th percentile: %s **"
        "\n** 95th percentile: %s **"
        % (arrMin, arrMax, arrMean, arrMedian, modeVal, fifth, twentyFive, fifty, seventyFive, ninetyFive)
    )

    colMeans = np.mean(x, axis=0)
    colVars = np.var(x, axis=0)

    indexMaxMean = np.argmax(colMeans)
    indexMinMean = np.argmin(colMeans)
    indexMaxVar = np.argmax(colVars)
    indexMinVar = np.argmin(colVars)

    regionMaxMean = names[indexMaxMean]
    regionMaxVar = names[indexMaxVar]
    regionMinMean = names[indexMinMean]
    regionMinVar = names[indexMinVar]
    ...
    print(
        ...
        "\n** Highest Variance: %s **"
        "\n** Lowest Variance: %s **"
        % (regionMaxMean, regionMinMean, regionMaxVar, regionMinVar)
    )
